...
function StencilTest:onDraw()
	...
	lovr.graphics.cube('fill', 0, 0, 0, 0.125) -- Draw cube at 0,0,0 for comparison
	...
	lovr.graphics.setShader(cloudDiscard)
	for i=0,4 do
		local color = 0.75-i/8
		lovr.graphics.setColor(color, color, color, 1)
		lovr.graphics.plane('fill', 0, 0, -0.5-i/2, w*self.wscale*2, h*self.wscale*2)
	end
